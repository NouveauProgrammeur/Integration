...
def PrintProcedure():
    ...
    print("the length of curve, and volume and surface of solid of revolution about given line.", end ='')
...
def TakeInput():
    ...
    return y, lowlim, uplim
    

def SetUp(lowlim, uplim):
    n = 100
    dx = (uplim - lowlim)/n
    xi = []
    for i in range (n+1):
...
def Calculate(y, lowlim, uplim, dx, xi):
...
def main():
    PrintProcedure()
    y, lowlim, uplim = TakeInput()
    dx, xi = SetUp(lowlim, uplim)
    Calculate(y, lowlim, uplim, dx, xi)
